import React from 'react'
import { Container, Row, Col, Grid, Avatar, Spacer, Button, Dropdown } from '@nextui-org/react';
import { AiOutlineHome, AiOutlineCalendar } from 'react-icons/ai';
import { BiChat } from 'react-icons/bi';
export const Navbar = (props: {}) => {
    return (
        <Grid.Container justify='flex-end' md={1} style={{
            boxShadow: '0px 0px 24px rgba(0, 0, 0, 0.08)', height: '100vh'
        }} >
            <Col >
                <Spacer y={2} />
                <Container >
                    <Dropdown>

                        <Dropdown.Trigger >
                            <Avatar size='xl' squared src='data:image/jpeg;base64,' />
                        </Dropdown.Trigger>
                        <Dropdown.Menu
                            variant="light"
                            aria-label="Actions"
                        >
                            <Dropdown.Item key="account">Account</Dropdown.Item>
                            <Dropdown.Item key="group" withDivider>Privacy</Dropdown.Item>
                        </Dropdown.Menu>
                    </Dropdown>

                </Container>
                <Spacer y={3} />
                <Button css={{ backgroundColor: 'white' }} auto>
                    <AiOutlineHome size={40} color='black' />
                </Button>

                <Spacer y={1} />
                <Button css={{ backgroundColor: 'white' }} auto>
                    <BiChat size={40} color='black' />
                </Button>
                <Spacer y={1} />
                <Button css={{ backgroundColor: 'white' }} auto>
                    <AiOutlineCalendar size={40} color='black' />
                </Button>
            </Col>

        </Grid.Container >
    )
}
